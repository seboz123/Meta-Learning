...
class ActorCritic(nn.Module):
    def __init__(self, device, state_dim, action_dim, hidden_size: int = 256):
        ...
        self.out_layers = [nn.Sequential(nn.Linear(hidden_size, shape).to(self.device), nn.Softmax(dim=-1)) for shape in action_dim]
    ...
    def act(self, state: np.ndarray):
        ...
        return action.detach().cpu().numpy(), dist.log_prob(action).detach().cpu().numpy()
...
class PPO_Meta_Learner:
    ...
    def calc_loss(self, memory_n, n_step: int = 1) -> torch.Tensor:
        use_n_step = True if n_step > 1 else False

        np.random.sample()
        samples = memory_n.sample_batch(128)
        print(samples)

        surrogate_loss = min(ratio, clip(pi_theta/pi_theta_k, 1 + epsilion, 1 - epsilon) * adv_pi_theta_k)
        torch.exp()
    ...
    def generate_and_fill_buffer(self, buffer_size, n_step, batch_size, task, max_trajectory_length = 600, gamma: float = 0.99):
        ...
        while buffer_length < buffer_size:
            if first_iteration:
                ...
                for brain in env.behavior_specs:
                    ...
                    for i in range(num_agents):
                        ...
                        transitions[i] = {
                            'obs_buf': np.zeros([max_trajectory_length, self.obs_space], dtype=np.float32),
                            ...
                            'act_log_prob_buf': np.zeros([max_trajectory_length, len(self.action_space)], dtype=np.float32),
                    ...
                    for agent_id_decisions in decision_steps:
                        ...
                        action, action_log_prob = self.actor_critic.act(np.array(init_state))
                        ...
                        transitions[agent_id_decisions]['act_log_prob_buf'][0] = action_log_prob
                    ...
                    for agent_id_terminated in terminal_steps:
                        ...
                        transitions[agent_id_terminated]['act_log_prob_buf'][0] = action_log_prob
            ...
            for agent_id in next_experiences:
                ...
                if not done:  # If the corresponding agent is not done yet, select and action and continue
                    ...
                    next_action, action_log_prob = self.actor_critic.act(np.array(next_obs))
                    ...
                    transitions[agent_id]['acts_buf'] = np.zeros([max_trajectory_length, len(self.action_space)],
                                                                 dtype=np.float32)
        ...
        memory_n = ReplayBuffer(
            self.obs_space, buffer_size,len(self.action_space),batch_size, n_step=n_step, gamma=gamma
        )
        ...
        for ptr in range(len(rews_buf)):
            ...
            memory_n.store(*transition)

        print("Finished generating Buffer with size of: {} in {:.3f}s!".format(len(memory_n), time.time() - start_time))
...
memory_n, buffer_size = ppo_module.generate_and_fill_buffer(1000, 128, 128, 0, 600)
ppo_module.calc_loss(memory_n, 64)